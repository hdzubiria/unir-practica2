        ...
        #!/bin/bash
        mkdir -p $HOME/clouddrive/crear-infraestructura/.TFTesting
...
resource \"azurerm_container_group\" \"TFTest\" {
    ...
    container {
        ...
        command = \"/bin/bash -c '/module/crear-infraestructura/.TFTesting/containercmd.sh'\"
...
}" > $HOME/clouddrive/crear-infraestructura/.TFTesting/testfile.tf
        ...
        mkdir -p $HOME/clouddrive/crear-infraestructura/.TFTesting/.azure

        az account list --refresh &> /dev/null

        cp $HOME/.azure/*.json $HOME/clouddrive/crear-infraestructura/.TFTesting/.azure
